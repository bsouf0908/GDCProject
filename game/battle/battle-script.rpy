...
define bt = Character(None, callback=narrator, ctc="ctc_blink", ctc_position="nestled") #you can replace the textbox image, font, whatever else
...
label battle_lowhp:
    bt "Things are starting to look bad for [povname]...!"
...
label battle_won:
    ...
    if not playerLV==10: # level cap
        $ playerEXP += enemy.EXP
        bt "[povname] gains [enemy.EXP] experience points!"
...
label levelup:
    ...
    bt "[povname] is now level [playerLV]!"
    # increase stats
